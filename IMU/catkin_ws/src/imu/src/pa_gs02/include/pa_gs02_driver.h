...
public:
    ...
    Parse_GPS():nh_get("~")
    {
        ...
        read_pub = nh.advertise<sensor_msgs::Imu>("/pa_gs02/imu/data",1);//串口接收  //发布数据
        ...
        ros::Rate loop_rate(4400);//指定循环的频率 
        ...
        while(ros::ok())
        {
            ...
            {
                ROS_INFO_STREAM("serial port not available");
            }
    ...
    bool ParsePacketB4(IMU_DATA_structure* imuData, unsigned char* buffer, int length)
    {
        ...
        uint8_t dataFreq = 0x3 & (unsigned char)buffer[53];
        uint8_t satNum = 0xf0 & (unsigned char)buffer[53];
        // cout<<buffer[53]<<endl;
        // cout<<"data freq: " << imuData->flag_frequency<<","<<imuData->flag_Magn<<","<<imuData->flag_Hbar<<", satellites num: "<<imuData->Satellites_num<<endl;
        printf("buffer 53:%#x, data freq:%d, sat num:%d\n", buffer[53], dataFreq, satNum);
        // TODO 这里是空的，看看是不是因为没天线的原因
        imuData->Pos_type = (unsigned char)buffer[54];
        // cout<<"pos type:" << buffer[54]<<endl;
        printf("pos type:%#x\n", buffer[54]);
